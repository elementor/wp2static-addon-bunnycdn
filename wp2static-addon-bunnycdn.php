<?php

/**
 * Plugin Name:       WP2Static Add-on: BunnyCDN Deployment
 * Plugin URI:        https://wp2static.com
 * Description:       BunnyCDN deployment add-on for WP2Static.
 * Version:           1.0-alpha-001
 * Author:            Leon Stafford
 * Author URI:        https://ljs.dev
 * License:           Unlicense
 * License URI:       http://unlicense.org
 * Text Domain:       wp2static-addon-bunnycdn
 */

if ( ! defined( 'WPINC' ) ) {
    die;
}

define( 'WP2STATIC_BUNNYCDN_PATH', plugin_dir_path( __FILE__ ) );
define( 'WP2STATIC_BUNNYCDN_VERSION', '1.0-alpha-006' );

if ( file_exists( WP2STATIC_BUNNYCDN_PATH . 'vendor/autoload.php' ) ) {
    require_once WP2STATIC_BUNNYCDN_PATH . 'vendor/autoload.php';
}

function run_wp2static_addon_bunnycdn() {
    $controller = new WP2StaticBunnyCDN\Controller();
    $controller->run();
}

register_activation_hook(
    __FILE__,
    [ 'WP2StaticBunnyCDN\Controller', 'activate' ]
);

register_deactivation_hook(
    __FILE__,
    [ 'WP2StaticBunnyCDN\Controller', 'deactivate' ]
);

run_wp2static_addon_bunnycdn();

